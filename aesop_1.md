...
홈 화면은 이솝 브랜드 무드 전달을 중심으로 구성되는 **에디토리얼 스크롤
구조**를 따른다.\
상단 Hero 연출 이후 스크롤을 따라 선(Line) 기반의 그래픽이 이어지며
브랜드 스토리, 철학, 성분, 제품, Gift 영역이 순차적으로 나타나는 구조로
구성한다.

### 12-1. Hero Intro 시퀀스

홈 화면 진입 시 첫 화면은 **어두운 톤의 Intro 화면**으로 시작한다.

구성

-   어두운 색상의 배경
-   흐리게 처리된 배경 이미지
-   제품은 아직 명확하게 보이지 않는 상태

목적

-   브랜드 분위기 전달
-   다음 연출로 자연스럽게 이어지는 도입 화면

### 12-2. Bottle Line Drawing

스크롤 또는 자동 진행 시 **병의 외곽선을 따라 라인이 그려지는 연출**이
진행된다.

연출 규칙

-   병의 외곽선을 따라 선이 그려진다
-   라인은 위에서 아래로 이어지며 병 형태를 완성한다
-   배경은 어두운 무드를 유지한다

### 12-3. Hero Product Reveal

병 외곽선이 완성되면 **실제 Hero 제품 이미지가 등장한다.**
...
Aesop / Origin 텍스트가 병 뒤에서 좌우로 퍼지듯 등장한다.

### 12-4. Hero Caption

Hero 타이포 아래에는 **작은 브랜드 메시지 문구**가 나타난다.

구성

-   Aesop 타이포 아래 작은 설명 문구 배치
-   브랜드 철학 메시지 전달

### 12-5. 브랜드 타임라인 섹션

Hero 이후 스크롤 시 **선(Line) 그래픽을 따라 브랜드 스토리가 이어지는
섹션**이 등장한다.

다음 세 항목을 **하나의 섹션으로 구성한다.**
...
-   스크롤 시 세로 선이 그려지며 아래로 확장
-   선을 따라 각 항목이 순차적으로 등장
-   아이콘 또는 일러스트와 함께 브랜드 스토리 설명

### 12-6. 브랜드 철학 섹션

다음 4개 항목을 **하나의 섹션으로 구성한다.**
...
-   스크롤 시 선 그래픽이 이어지며 각 블록 등장
-   이미지와 텍스트 교차 레이아웃

### 12-7. 성분 스토리 섹션

이 섹션에서는 **성분 스토리와 라인 인터랙션 연출**을 사용한다.

연출

-   스크롤 진행에 따라 선이 그려짐
-   선을 따라 성분 요소 등장
-   마지막에는 선이 병 외곽 라인으로 연결

### 12-8. 제품 전환 연출

성분 라인이 병 외곽 라인으로 연결된 이후 **제품 연출**이 진행된다.
...
2.  실제 제품 이미지가 나타난다\
3.  제품이 왼쪽으로 밀려난다\
4.  하단에서 다른 제품이 올라오며 다음 제품으로 전환된다

### 12-9. Best Selection 섹션

베스트 섹션은 대표 제품을 보여주고 **베스트 상품 리스트 페이지로
이동시키는 CTA 섹션**이다.
...
Best 이후 **브랜드 메시지를 강조하는 대형 타이포 섹션**
...
Gift Guide 섹션은 **Gift 페이지로 이동하는 CTA 영역**이다.
...
### 12-12. 대표 기프트 3제품 바로가기 섹션

Gift 소개 이후 **대표 기프트 제품 3개를 바로 이동할 수 있는 섹션**
...
`/product/:id` 상세 페이지 이동

### 12-13. 마무리 메시지 섹션

홈 화면 마지막에는 **브랜드 메시지로 마무리되는 섹션**을 배치한다.
...
Aesop / Origin Hero\
↓\
Brand Timeline (Genesis / Heritage / Presence)\
↓\
Brand Philosophy (Nature Refined \~ Ethical Commitments)\
↓\
Ingredient Story\
↓\
Bottle Reveal + Product Transition\
↓\
Best Selection\
↓\
Message Section\
...
홈 화면의 Hero 연출과 스크롤 기반 인터랙션은 **GSAP(GreenSock)**을
기준으로 구현한다.

사용 목적

-   Hero Intro 연출
-   병 외곽선 Line Drawing 애니메이션
-   Hero 제품 등장 애니메이션
-   `Aesop / Origin` 타이포 확장 연출
-   브랜드 스토리 섹션의 Line 애니메이션
-   성분 섹션 스크롤 기반 라인 드로잉
-   제품 전환 애니메이션
-   섹션별 순차 등장 애니메이션
...
-   스크롤 위치를 기준으로 애니메이션 진행 상태를 제어한다.
-   선(Line) 애니메이션은 **SVG Path 드로잉 방식**으로 구현한다.
-   섹션 전환은 과도한 모션보다 자연스러운 흐름을 우선한다.
-   브랜드 무드를 해치지 않도록 easing과 duration은 절제된 값을
    사용한다.

모바일 대응

-   모바일 환경에서는 성능을 고려하여 일부 애니메이션을 단순화할 수
    있다.
-   ScrollTrigger의 pin 사용 시 모바일 스크롤 성능을 반드시 테스트한다.

정합 원칙

-   홈 화면 애니메이션은 **브랜드 경험 전달이 목적**이며 과도한 모션을
    지양한다.

-   핵심 구간(Hero / Line Drawing / Product Transition)에 애니메이션을
    집중한다.

-   상품 나열 중심 구조가 아니라 **브랜드 경험 중심 구조**

-   선(Line) 그래픽이 전체 스토리를 연결

-   제품 노출은 후반부에서 자연스럽게 등장
...
src/
  ...
  store/
    ...
    useStore.js        # slices들을 Root로 엮고 ff-storage 키값으로 persist 선언
  styles/
